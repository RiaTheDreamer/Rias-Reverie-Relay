...
export const R45_UTILITY_CONTRACTS: Record<R45ActiveSurfaceId, string> = {
  ...
  smartphone: utility('smart_phone', 'Child order: optional <notifications>, optional <contact>, required <messages>, optional <info>. Contact and info are text-only. Inside <messages>, preserve conversation order. Text uses <s_recv> or <s_sent>. Every <s_recv> must close with </s_recv>; every <s_sent> must close with </s_sent>; all <s_recv>, <s_sent>, and <s_img> children must remain inside <messages>. Do not invent alternate message closing structures. Every image message MUST be <s_img side="sent|recv" time="…"><image_request id="…" target="smartphone.message-image" slot="…" aspect="4:3" alt="…"><scene_brief>Exact contextual attachment only; no phone UI or readable text.</scene_brief></image_request></s_img>. side="sent" is user/right; side="recv" is contact/left. Never author a direct-child image_request under <messages> and never omit side on new <s_img>.'),
