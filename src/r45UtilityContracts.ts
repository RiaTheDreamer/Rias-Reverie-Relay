...
export const R45_UTILITY_CONTRACTS: Record<R45ActiveSurfaceId, string> = {
  ...
  'dating-profile': utility('tinder', 'Author exactly three [profile] records. Each record carries [slot], [prev], and [next] child fields. Each [photo] owns one 3:4 identity image: centered face and upper torso, full hair visible, generous headroom, current outfit/context, no extreme close-up. The same source is reused in match states, so it must be crop-safe.'),
  'public-bulletin': utility('public_bulletin', 'Author [authority], [level], [headline], [timestamp], [district], [pb_media], [pb_body], and [pb_instructions] in order. [pb_media] owns one 16:9 documentary image of the exact event/place; all readable bulletin text remains in bracket fields.'),
  ...
  'naver-article': utility('naver_news', 'Author [nv_media], one [nv_body], then [nv_comments]. Hero media is a 16:9 editorial image of the exact story event/place/person; article and comment text remain in bracket fields.'),
...
All [s_recv], [s_sent], and [s_img] rows remain inside [messages]. Every [s_img] contains [side], [time], then one <image_request id="…" target="smartphone.message-image" slot="…" aspect="4:3" alt="…"><scene_brief>Exact contextual attachment only; no phone UI or readable text.</scene_brief></image_request>. sent is user/right; recv is contact/left. Images are allowed only inside a message row; never place media in contact or info.`),
  ...
  'court-transcript': utility('court_transcript', 'Author case/court/status/time/judge fields, ordered [ct_lines], [objection], and [exhibit]. Preserve clear speaker/time/body semantics; do not turn the transcript into a generic image Surface.'),
  'codex-entry': utility('codex_entry', 'Author [title], [type], [region], [status], [era], [codex_media], [body], [facts], and related-reference fields. Media is a 4:3 encyclopedia-appropriate subject/place/object image.'),
  ...
  'twitter-profile': utility('twitter_profile', 'Author profile child fields, [twip_avatar], [twip_cover], [twip_posts], and [twip_media_grid]. Avatar is reusable 1:1; cover is 3:1; post avatars are reusable 1:1; tweet media is 16:9 unless context requires 4:3; media-grid items are 1:1. Keep post child order avatar, text, media, comments, thread. Tabs are Posts and Media only.'),
