...
.dgir-prose-lifecycle-projection[data-dgir-prose-size="full"]>.rrl-island{flex:1 1 100%;width:100%;max-width:none}
...
function proseLifecycleProjection(record: NonNullable<NativeSurfaceRenderContext['records']>[number] | undefined, card: string): string {
  const alignment = record?.proseImageAlignment === 'left' || record?.proseImageAlignment === 'right'
    ? record.proseImageAlignment
    : 'center'
  const size = record?.proseImageSize === 'small' || record?.proseImageSize === 'large' || record?.proseImageSize === 'full'
    ? record.proseImageSize
    : 'medium'
  const width = size === 'small' ? '48%' : size === 'large' ? '84%' : size === 'full' ? '100%' : '66%'
  const maxWidth = size === 'small' ? '420px' : size === 'large' ? '920px' : size === 'full' ? '100%' : '720px'
  const justify = alignment === 'left' ? 'flex-start' : alignment === 'right' ? 'flex-end' : 'center'
  const key = record?.key ? ` data-dgir-prose-projection="${escapeAttr(record.key)}"` : ''
  return `<div class="dgir-prose-lifecycle-projection"${key} data-dgir-prose-align="${alignment}" data-dgir-prose-size="${size}" style="--dgir-prose-image-width:${width};--dgir-prose-image-max-width:${maxWidth};--dgir-prose-image-justify:${justify}">${lifecycleCardIsland(card)}</div>`
}
...
export function renderNativeSurfaceMarkup(
  ...
  content = content.replace(/<scene_image\b([^>]*)>([\s\S]*?)<\/scene_image>/gi, (full, rawAttrs, body) => {
    ...
    const record = currentRequestRecord({ context: renderContext, requestId })
    return proseLifecycleProjection(record, renderRequestCard({
      title: 'Illustration requested',
      brief: stripMarkup(String(body || '')),
      requestId,
      aspect: attrs.aspect || record?.requestAspect || '4:3',
      rootTag: 'scene_image',
  ...
  content = content.replace(/<reverie-illustration\b([^>]*)>([\s\S]*?)<\/reverie-illustration>/gi, (_full, rawAttrs, body) => {
    ...
    renderedCount += 1
    renderedSurfaceIds.push('prose-illustration')
    const requestId = attrs.slot || attrs.id || ''
    return proseLifecycleProjection(currentRequestRecord({ context: renderContext, requestId }), renderRequestCard({
      title: 'Illustration requested',
      brief: stripMarkup(String(body || '')),
      requestId,
...
export function renderGenerationPlaceholderEffect(effect: GenerationPlaceholderEffect): string {
...
function stableLifecycleMediaSlot(aspect: string, state: string, title: string, inner = '', empty = true, effect?: GenerationPlaceholderEffect): string {
