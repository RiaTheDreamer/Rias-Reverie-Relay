...
.rrl-media-slot .rrl-slot-image.rrl-final-reveal{animation:rrlFinalReveal 1.2s cubic-bezier(.2,.68,.2,1) both;will-change:filter,opacity,transform}@keyframes rrlFinalReveal{0%{opacity:.35;transform:scale(1.01);filter:blur(16px) brightness(.94)}52%{opacity:.86;filter:blur(6px) brightness(.99)}100%{opacity:1;transform:none;filter:none}}
...
function proseLifecycleProjection(record: NonNullable<NativeSurfaceRenderContext['records']>[number] | undefined, card: string): string {
  ...
  // Image Size and alignment are live presentation preferences, not immutable
  // generation metadata. Inherit the document-level 0.2.8.6.4 variables so
  // active Status Cards and completed images update together immediately.
