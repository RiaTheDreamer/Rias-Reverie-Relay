...
assert(![backend, frontend].some(source => /dreamglass[_-]image[_-]router/i.test(source)), 'public runtime must not retain the retired Dreamglass Image Router identity')
...
const prosePatch = frontend.slice(frontend.indexOf('function patchProseSettings('), frontend.indexOf('function sendProseAction('))
assert(prosePatch.includes('applyGlobalInterfaceSettings()'), 'Image Size and alignment must update presentation immediately instead of waiting for a backend echo')
assert(backend.includes("proseImageAlignment: plan.imageAlignment || 'center'") && backend.includes("proseImageSize: plan.imageSize || 'medium'"), 'prose image presentation must be retained on the generated slot record')
...
assert(frontend.includes('if (Object.keys(settings).length === 0) return null'), 'empty Native ImageGen responses must not be cached as valid settings snapshots')
assert(frontend.includes("Load Relay's persisted state before any write-capable native-settings sync") && frontend.includes('void refreshState(false)') && frontend.includes('if (!nativeSettingsLastSyncedAt) void syncNativeSettings(true)'), 'cold frontend boot must hydrate Relay state before its first write-capable native settings sync')
assert(backend.includes('reverie-relay-config-probe-') && backend.includes('for (const delayMs of [0, 40, 120, 360, 800])') && backend.includes('Native settings sync deferred until persisted Relay configuration is available.'), 'cold config reads must use bounded storage-readiness retries and native sync must fail closed instead of persisting an immediate fallback')
assert(frontend.includes('nativeSnapshotScanAttempts') && frontend.includes('attempt >= 7 ? 10_000'), 'Native ImageGen snapshot recovery must use bounded backoff before continuing a low-frequency automatic retry')
assert(frontend.includes('scheduleTerminalStateRefresh(message.chatId)'), 'terminal image events must refresh authoritative slot state without requiring the drawer to reopen')
assert(frontend.includes("const stallEligible = ['preparing', 'parsing', 'generating', 'previewing', 'placement-pending']"), 'manual or settings-waiting queued slots must not be mislabeled as stalled generation')
assert(frontend.includes(".rrcp-presentation-glass > .rrcp-launch") && frontend.includes("shell.style.setProperty('display', open ? 'block' : 'none', 'important')"), 'Character Phone outer launchers, including Glass, must receive host-safe interaction binding without changing supplied Regex styling')
assert(frontend.includes("label: 'Glass Button'") && frontend.includes("label: 'Glass Mode'") && frontend.includes("id: 'glass', label: 'Glass Mode'"), 'Glass Button presentation and Glass Mode color controls must be independent')
assert(frontend.includes("id: 'plain-glass', label: 'Plain Glass'") && frontend.includes("['plain-glass', 'Plain Glass']"), 'Plain Glass must be selectable globally and in Surface shell settings')
assert(frontend.includes('event.composedPath().find(node => node instanceof HTMLDetailsElement)') && frontend.includes('nodeBelongsToMountedRoot'), 'Surface disclosure restoration must survive shadow-scoped host remounts after image insertion')
assert(frontend.includes("'.rrcp-app-launch'") && frontend.includes("page.style.setProperty('display', open ? 'block' : 'none', 'important')"), 'Character Phone app launchers must remain navigable when host label toggling is unavailable')
...
console.log('0.2.8 UX, macro, modal, Surface overview, and LoRA discovery smoke ok')
