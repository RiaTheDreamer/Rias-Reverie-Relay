...
assert(finalRevealSource.includes('image.complete && image.naturalWidth > 0') && finalRevealSource.includes('await image.decode?.()') && finalRevealSource.includes("image.addEventListener('load', reveal, { once: true })"), 'final reveal must wait for a usable loaded or decoded image')
...
assert(finalRevealSource.includes("window.matchMedia?.('(prefers-reduced-motion: reduce)').matches") && finalRevealSource.includes("image.addEventListener('animationend'") && finalRevealSource.includes("image.classList.remove('rrl-final-reveal')"), 'final reveal must skip reduced motion and remove its one-shot class on animation end')
assert(!/MutationObserver|setInterval|setTimeout/.test(finalRevealSource), 'final reveal must not introduce observers or polling timers')
assert(nativeSource.includes('.rrl-media-slot .rrl-slot-image.rrl-final-reveal{animation:rrlFinalReveal .42s') && nativeSource.includes('@keyframes rrlFinalReveal{from{opacity:0;transform:scale(.99);filter:blur(5px) brightness(1.04)}to{opacity:1;transform:none;filter:none}}'), 'shared stable media CSS must own the subtle final-image-only reveal')
