...
assert(nativeSource.includes('.rrl-media-slot .rrl-slot-image.rrl-final-reveal{animation:rrlFinalReveal 1.2s cubic-bezier(.2,.68,.2,1) both;will-change:filter,opacity,transform}') && nativeSource.includes('@keyframes rrlFinalReveal{0%{opacity:.35;transform:scale(1.01);filter:blur(16px) brightness(.94)}52%{opacity:.86;filter:blur(6px) brightness(.99)}100%{opacity:1;transform:none;filter:none}}'), 'shared stable media CSS must own the slow final-image-only unblur reveal')
