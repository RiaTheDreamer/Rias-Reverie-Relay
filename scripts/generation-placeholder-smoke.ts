...
assert(!/MutationObserver|setInterval|setTimeout/.test(`${finalRevealLifecycleSource}\n${finalRevealSource}`) && (visualSessionSource.match(/setInterval/g) || []).length === 1 && visualSessionSource.includes('sendFrontendSession(true, true)'), 'final reveal must use event synchronization; its separate bounded recovery path may only use the in-flight frontend-session lease heartbeat')
