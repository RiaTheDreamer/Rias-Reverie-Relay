...
assert.match(read('src/build.ts'), /EXTENSION_VERSION = '0\.2\.8'/)
assert.match(read('src/build.ts'), /BUILD_ID = '\d{8}-0\.2\.8'/)
...
console.log('0.2.8 release smoke passed: version metadata aligned, authority hashes verified, and a 0.2.7.5-era schema-34 completed image/state snapshot migrated without losing historical media.')
