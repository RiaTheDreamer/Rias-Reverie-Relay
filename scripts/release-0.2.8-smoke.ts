...
assert.equal(pkg.version, '0.2.8.7.2')
assert.equal(manifest.version, '0.2.8.7.2')
assert.match(read('README.md'), /\*\*Version:\*\* `0\.2\.8`/)
assert.match(read('src/build.ts'), /EXTENSION_VERSION = '0\.2\.8\.7\.2'/)
assert.match(read('src/build.ts'), /BUILD_ID = '20260924-0\.2\.8\.7\.2'/)
...
console.log('0.2.8.7.2 release smoke passed: version metadata aligned, authority hashes verified, and a 0.2.7.5-era schema-34 completed image/state snapshot migrated without losing historical media.')
