...
assert(backend.includes('releaseAbortedImageGenerationLane') && backend.includes('serialized lane quarantined until the host transport settles'), 'timeout/cancel does not quarantine the serialized provider lane until host settlement')
assert(backend.includes('late result discarded; serialized lane is safe to release'), 'late provider settlement does not explicitly release the quarantined lane')
