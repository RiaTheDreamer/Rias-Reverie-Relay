...
assert(backend.includes('await acquireImageGenerationLane(userId, context, controller, plan.provider)'), 'ImageGen calls do not acquire the provider-aware shared serialized lane')
...
assert(backend.includes('export const IMAGE_GENERATION_TIMEOUT_MS: number | undefined = undefined'), 'production provider generation must remain timeoutless')
assert(backend.includes('export const IMAGE_GENERATION_LANE_WAIT_TIMEOUT_MS: number | undefined = undefined'), 'production provider-lane waiting must remain timeoutless')
assert(backend.includes('if (!Number.isFinite(timeoutMs)) return await Promise.race([operation(), aborted])'), 'optional deterministic test deadline no longer defaults to timeoutless production behavior')
assert(backend.includes('releaseAbortedImageGenerationLane') && backend.includes('serialized lane quarantined until the host transport settles'), 'timeout/cancel does not enter the serialized provider drain quarantine')
assert(backend.includes('lane.drainWatchdog = setTimeout') && backend.includes("providerDetachReason = 'automatic-drain-deadline'"), 'abandoned provider work has no bounded automatic detach path')
