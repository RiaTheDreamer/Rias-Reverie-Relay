...
assert(frontendSource.includes("imageSize === 'full') document.documentElement.style.setProperty('--dgir-prose-bubble-inner-width', '100%')") && frontendSource.includes("removeProperty('--dgir-prose-bubble-inner-width')"), 'Full Width must remove the prose bubble inset while smaller live settings restore their own clamps')
assert(frontendSource.includes('var(--dgir-prose-bubble-inner-width, calc(100% - (2 * clamp(18px, 3vw, 38px))))'), 'prose sizing must preserve the historical non-full bubble inset')
