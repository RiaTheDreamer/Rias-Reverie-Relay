...
assert(frontendSource.includes(':is(span, a):has(> img[data-dgir-app="prose"][data-dgir-prose-size="full"])') && frontendSource.includes('img[data-dgir-app="prose"][data-dgir-prose-size="full"] { flex-basis: 100%'), 'Full Width must fill only the bound prose image and its sanitized image wrapper')
assert(!frontendSource.includes("setProperty('--dgir-prose-bubble-inner-width'") && !frontendSource.includes('[data-component="BubbleMessage"] > div[class*="bubble"] > div[class*="content"]:has(img'), 'Image Size must not widen prose, MessageContent, or BubbleMessage')
