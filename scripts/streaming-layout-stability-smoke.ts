...
assert(frontendSource.includes('mediaSlot.dataset.rrnMediaState') && frontendSource.includes('prepareFinalLifecycleImage(slotImage, visualImageUrl, shouldReveal || waitingForCompletedRender, urlMatches)'), 'frontend must hydrate the existing stable slot while concealing reveal-bound final pixels until decode')
assert(frontendSource.includes('record.pendingPlacement?.imageUrl || record.imageUrl') && frontendSource.includes('revealFinalImageWhenReady(card, slotImage, visualImageUrl') && frontendSource.includes('readyForReveal: !waitingForCompletedRender'), 'final pixels must hydrate in the existing slot but wait for durable completed projection before Reveal')
...
assert(frontendSource.includes('requestedProjectionInvalidations') && frontendSource.includes("ctx.display?.invalidate(['*'])"), 'a missing mounted media owner must use Lumiverse display invalidation rather than waiting for page refresh')
...
assert(frontendSource.includes('invalidateDisplayIfContractChanged') && (frontendSource.match(/ctx\.display\?\.invalidate\(\['\*'\]\)/g) || []).length === 2 && frontendSource.includes('PROJECTION_INVALIDATION_MAX_ATTEMPTS = 3'), 'ordinary slot-state updates must not remount every Surface; only a bounded missing-media repair may use the host wildcard')
...
assert(nativeSource.includes('.dgir-prose-lifecycle-projection .rrl-media-slot .rrl-preview-image,.dgir-prose-lifecycle-projection .rrl-media-slot .rrl-slot-image,.dgir-prose-lifecycle-projection .rrl-media-slot .rrl-resolved img{object-fit:cover}'), 'prose lifecycle preview and final media must cover only their stable prose slot')
assert(frontendSource.includes('.dgir-prose-lifecycle-projection .rrl-media-slot :is(img.rrl-slot-image[data-dgir-app="prose"],img.rrl-preview-image)') && frontendSource.includes('object-fit: cover !important;'), 'frontend prose lifecycle media must preserve cover through preview and final hydration')
...
assert(!backendSource.includes("spindle.on('CHARACTER_MESSAGE_RENDERED'"), 'opening an old chat must never scan or regenerate Relay slots from a render event')
assert(backendSource.includes("spindle.on('MESSAGE_SENT'") && backendSource.includes("spindle.on('GENERATION_ENDED'"), 'fresh generation events must remain the sole automatic Relay discovery path')
...
assert(backendSource.includes('withPlacementMutationLock(job') && backendSource.includes("return `${relayQueueScope(userId)}:${job.chatId}:${job.messageId}:${job.swipeId}`"), 'initial siblings must serialize projection without waiting for generation completion')
