...
assert(frontendSource.includes('record.pendingPlacement?.imageUrl || record.imageUrl') && frontendSource.includes('revealFinalImageWhenReady(card, slotImage, visualImageUrl'), 'placement-pending final pixels must hydrate and reveal in the existing slot before canonical persistence')
assert(frontendSource.includes('revealedFinalImageByRecord') && frontendSource.includes('rememberBoundedMap(revealedFinalImageByRecord'), 'a coalesced host remount must not replay Reveal for an image already revealed from pending state')
...
assert(backendSource.includes('pendingPlacementBatches') && backendSource.includes('stageGeneratedPlacement(job, results') && backendSource.includes('commitInitialPlacementBatch(batch'), 'successful siblings must enter one message-scoped persistence transaction')
...
assert(backendSource.includes('skipChunkRebuild: true') && backendSource.includes("if (swipeId === activeSwipeId(message)) return { content") && backendSource.includes('const swipes = [...message.swipes]'), 'active media persistence must be content-only while inactive swipes remain explicit')
...
assert(backendSource.includes('hasPendingInitialPlacementSibling') && backendSource.includes("return `${relayQueueScope(userId)}:${job.chatId}:${job.messageId}:${job.swipeId}`"), 'initial siblings must wait behind one message/swipe durable placement transaction')
